...
const initMonthChangers = () => {
    const iconBack = document.querySelector(".fa-backward-step");
    const iconForvard = document.querySelector(".fa-forward-step");
...
const handleDaySelect = (event) => {
    ...
    getDayTimeSlots(event.target.dataset.day, 1);
}

const getDayTimeSlots = (date, interval) => {
    ...
    for (let i = FIRST_HOUR; i <= LAST_HOUR; i++) {
        ...
        td1.textContent = `${i}:00 - ${i + interval}:00`;
        tr.appendChild(td1);
